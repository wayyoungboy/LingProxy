...
- **Token管理**: 请求端Token管理，支持策略关联和API密钥认证
- **CORS支持**: 灵活的跨域资源共享配置
- **安全存储**: API密钥和密码加密存储

### 📊 管理功能
- **管理员管理**: 单管理员模式，支持密码和API密钥管理
- **Token管理**: 创建和管理请求端Token，支持策略绑定
- **策略管理**: 内置多种路由策略模板（随机、轮询、加权、模型匹配、正则匹配、优先级、故障转移），支持自定义策略实例
- **LLM资源管理**: 支持基于驱动的AI服务资源配置（目前支持OpenAI驱动），支持模型类别（对话、生图、嵌入、重排序、语音、视频），支持Excel模板批量导入/导出
- **模型管理**: 灵活的模型配置，支持定价、使用限制等参数
- **系统设置**: 动态配置管理，包括基础设置、缓存、限流、安全、日志、负载均衡、熔断器等配置
...
- `GET /api/v1/tokens/:id` - 获取Token详情
- `POST /api/v1/tokens` - 创建Token
- `PUT /api/v1/tokens/:id` - 更新Token
- `DELETE /api/v1/tokens/:id` - 删除Token
- `POST /api/v1/tokens/:id/reset` - 重置Token
- `PUT /api/v1/tokens/:id/policy` - 为Token绑定策略
- `DELETE /api/v1/tokens/:id/policy` - 移除Token的策略绑定
...
#### 统计信息
- `GET /api/v1/stats/system` - 获取系统统计信息
- `GET /api/v1/stats/llm-resources/:id` - 获取LLM资源统计信息
...
// Token Token模型 - 请求端Token管理
type Token struct {
    ID         string     // Token唯一标识
    Name       string     // Token名称/描述
    Token      string     // Token值（API Key，以"ling-"开头）
    Prefix     string     // Token前缀（用于显示）
