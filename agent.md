...
2. **API Key 管理**：请求端 API Key 管理，支持策略绑定（"ling-" 前缀 API Key），支持 API Key 重置和复制
3. **LLM 资源管理**：
...
5. **自动重试机制**：可配置的自动重试功能，支持指数退避，自动重试网络错误、超时和5xx服务器错误
6. **系统设置**：动态配置管理（基础、缓存、限流、安全、日志、负载均衡、Provider重试）
7. **日志管理**：支持日志文件查看、下载、清理
8. **统计功能**：系统统计、LLM 资源使用统计、用户统计
9. **国际化支持**：前端支持中英文切换（Vue I18n）
10. **OpenAI 兼容 API**：`/llm/v1/*` 路径前缀
...
**API Key 管理功能**：
- API Key 生成：自动生成 "ling-" 前缀的 API Key
- API Key 策略绑定：支持为 API Key 绑定路由策略
- API Key 验证：在认证中间件中验证 API Key
...
**自动重试功能**：
- 重试逻辑：在 `service/openai_service.go` 中实现，使用指数退避策略
- 重试配置：可通过管理界面配置（设置 → Provider设置）
- 可重试错误：网络错误、超时、5xx服务器错误、429限流错误
- 不可重试错误：4xx客户端错误（429除外）、认证错误、上下文取消
- 适用范围：所有请求类型（聊天补全、文本补全、嵌入、流式）
- 配置项：`provider.max_retries`（默认：3），`provider.retry_delay`（默认：1s）
- 即时生效：配置修改后无需重启服务
...
- **认证问题**：检查认证中间件是否正确应用，API Key 验证逻辑
...
2. **API Key 前缀**：请求端 API Key 必须以 "ling-" 开头
3. **策略绑定**：只有 "ling-" 开头的 API Key 才能绑定策略
...
12. **API Key 前缀验证**：忘记验证 API Key 前缀，导致非 "ling-" 前缀的 API Key 被创建

## API 路径与常量配置

### 后端 API 路径
- **管理 API**：`/api/v1/*`（需要认证，除非认证开关关闭）
- **OpenAI 兼容 API**：`/llm/v1/*`（使用 API Key 认证）
- **健康检查**：`/health`（无需认证）
